...
var onLogTrigger = (runtime2, triggerData) => {
  ...
  const step_evm_read_1 = evmClient_ethereum_testnet_sepolia.read(runtime2, {
  ...
  const _authSecret_http_1 = runtime2.getSecret({ id: "VALIDATION_API_KEY" }).result();
  ...
  const step_http_1 = httpClient.sendRequest(runtime2, fetch_http_1, consensusIdenticalAggregation())(_fetchCfg_http_1).result();
  ...
  if (step_parse_1.isValid === "true") {
    ...
    const step_mint_1___write = evmClient_ethereum_testnet_sepolia_base_1.write(runtime2, {
...
var initWorkflow = (config) => {
  const network248 = getNetwork({ chainFamily: "evm", chainSelectorName: "ethereum-testnet-sepolia", isTestnet: true });
  if (!network248) {
